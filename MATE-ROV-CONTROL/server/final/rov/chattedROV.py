...
class ROV:
    ...
    def start_depth_thread(self):
        ...
        if not self.depth_sensor.available:
            logger.warning("Depth sensor not available, depth thread not started")
            return False
            
        self.depth_running = True
        self.depth_thread = threading.Thread(target=self._depth_updater)
        self.depth_thread.daemon = True
        self.depth_thread.start()
        logger.info("Depth update thread started")
        
        # Calibrate depth sensor at surface if it's available
        if not self.depth_initialized and self.depth_sensor.available:
            self.depth_sensor.calibrate_at_surface()
            self.depth_initialized = True
            # Initialize Z target with current depth
            self.chassis_control.z_target = 0.0  # Set to 0 since we calibrated at surface
