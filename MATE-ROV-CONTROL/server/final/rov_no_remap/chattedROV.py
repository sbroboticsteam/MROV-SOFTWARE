...
class ROV:
    ...
    def process_command(self, command_data):
        ...
        if 'controller' in command_data:
            ...
            # Only process on button press, not hold
            elif x_button == 1 and prev_x == 0 and previous_state != ArmState.STOWED:
                self.arm.set_state(ArmState.STOWED)
                command_processed = True
            elif y_button == 1 and prev_y == 0 and previous_state != ArmState.FULLY_OUT:
                self.arm.set_state(ArmState.FULLY_OUT)
                command_processed = True
            elif lb_button == 1 and prev_lb == 0 and previous_state != ArmState.OUT_DOWN:
                self.arm.set_state(ArmState.OUT_DOWN)
                command_processed = True
            elif rb_button == 1 and prev_rb == 0 and previous_state != ArmState.FULLY_DOWN:
                ...
                if dpad_x == -1:  # Left on D-pad
                    ...
                    self.arm.adjust_wrist(1)
                    command_processed = True
                elif dpad_y == -1:  # Left on D-pad
                    self.arm.adjust_claw(-1)
                    command_processed = True
                elif dpad_y == 1:  # Right on D-pad
                    self.arm.adjust_claw(1)
                    command_processed = True
