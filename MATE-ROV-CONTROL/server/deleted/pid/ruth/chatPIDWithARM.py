...
class Servo:
    """Class to control a servo using PCA9685"""
    ...
    def _set_pulse_width(self, pulse_width):
        ...
        pulse_width = int(max(self.min_pulse, min(self.max_pulse, pulse_width)))
...
class Arm:
    ...
    def __init__(self, pca, servo_channels=None):
        """Initialize the arm with all servos"""
        logger.info("Initializing Arm Control...")
        self.pca = pca
        self.current_state = ArmState.STOWED

        # Default servo channel assignments
        if servo_channels is None:
            servo_channels = {
                "claw": 12,      # Example channel 12
                "wrist": 13,     # Example channel 13
                "elbow": 14,     # Example channel 14
                "shoulder": 15   # Example channel 15
            }
        
        # Create servo objects with correct channel assignments and names
        self.servos = {
            "claw": Servo(servo_channels["claw"], pca, min_pulse=500, max_pulse=2100, name="Claw"),
            "wrist": Servo(servo_channels["wrist"], pca, min_pulse=900, max_pulse=2000, name="Wrist"),
            "elbow": Servo(servo_channels["elbow"], pca, min_pulse=850, max_pulse=1600, name="Elbow"),
            "shoulder": Servo(servo_channels["shoulder"], pca, min_pulse=900, max_pulse=1600, name="Shoulder")
        }
        ...
        logger.info("Arm Control initialized with servo channels: " + str(servo_channels))
        
    def initialize(self):
        """Initialize the arm to stowed position"""
    ...
    def process_controller_input(self, controller_data):
        """Process controller data to control the arm"""
        ...
        buttons = {
            'a': controller.get('a', 0),
            'b': controller.get('b', 0),
            'x': controller.get('x', 0),
            'y': controller.get('y', 0),
            'lb': controller.get('lb', 0),
            'rb': controller.get('rb', 0)
        }
        
        # Get D-pad for wrist rotation
        dpad_x = controller.get('dpad_x', 0)
        
        # Initialize prev_buttons if it's the first time
        ...
        # X button (stowed position) - on button press, not hold
        if buttons['x'] == 1 and self.prev_buttons['x'] == 0:
            self.set_state(ArmState.STOWED)
            
        # Y button (fully out position) - on button press, not hold
        if buttons['y'] == 1 and self.prev_buttons['y'] == 0:
            self.set_state(ArmState.FULLY_OUT)
            
        # RB (fully down position) - on button press, not hold
        if buttons['rb'] == 1 and self.prev_buttons['rb'] == 0:
            self.set_state(ArmState.FULLY_DOWN)
            
        # LB (out and down position) - on button press, not hold
        if buttons['lb'] == 1 and self.prev_buttons['lb'] == 0:
            self.set_state(ArmState.OUT_DOWN)
            
        # D-pad left/right for wrist rotation
        if dpad_x == -1:  # Left on D-pad
            self.adjust_wrist(-1)  # Rotate left
        elif dpad_x == 1:  # Right on D-pad
            self.adjust_wrist(1)   # Rotate right
            
        # Store current button states for next time
        self.prev_buttons = buttons.copy()
        
        return True
        
    def shutdown(self):
        """Move the arm to stowed position for safe shutdown"""
        logger.info("Shutting down arm control...")
        # Return to stowed position
        self.set_state(ArmState.STOWED)
        sleep(0.5)  # Give time for the arm to move
        logger.info("Arm shutdown complete")
...
# --------------------------- ROV Class ---------------------------
class ROV:
    """ROV control system with PID stabilization and arm control."""
    ...
    def __init__(self, pid_enabled=True, pid_weight=1.0, calibration_file=None):
        ...
        # Use servo channels that don't conflict with thrusters
        servo_channels = {
            "claw": 0,      # Example - adjust as needed
            "wrist": 1,     # Example - adjust as needed
            "elbow": 2,     # Example - adjust as needed
            "shoulder": 3   # Example - adjust as needed
        }
        self.arm = Arm(self.pca, servo_channels)
        ...
        # Control mode flag (movement vs. arm)
    ...
    def log_debug_info(self):
        ...
        debug_info = (
            ...
            f"Arm Control Mode: {self.arm_control_mode}\n"
            f"=============================\n"
        )
    ...
    def initialize_hardware(self):
        ...
        logger.info("All hardware initialized")
    ...
    def process_command(self, command_data):
        ...
        # Process controller data for arm control if in arm mode
        if self.arm_control_mode and 'controller' in command_data:
            self.arm.process_controller_input(command_data)
            command_processed = True
        
        # Process controller data for movement if not in arm mode
        elif 'controller' in command_data:
            controller = command_data['controller']
            
            # Check for arm mode toggle (e.g., pressing 'start' button)
            if controller.get('start', 0) == 1 and not self.prev_start_button:
                self.arm_control_mode = not self.arm_control_mode
                logger.info(f"Toggled arm control mode: {'ENABLED' if self.arm_control_mode else 'DISABLED'}")
                command_processed = True
            
            # Store start button state for edge detection
            self.prev_start_button = controller.get('start', 0)
            
            if not self.arm_control_mode:
                # Extract controller values for movement
                ...
                motor_values = arcadeDrive3(
                    left_x, 
                    -left_y,  # Invert Y-axis if needed
                    right_x,
        ...
        # Process other command types (PID settings, etc.)
        ...
        # Update the last command time if any command was processed
    ...
    def update_thrusters(self):
        ...
        # If in arm control mode, don't update thrusters
        if self.arm_control_mode:
            # Set all thrusters to zero when in arm control mode
            for thruster in self.thrusters:
    ...
    def run(self):
        ...
        try:
            ...
            # Initialize flag for button edge detection
            ...
            while self.running:
                ...
                # Update thruster values based on commands and PID (unless in arm mode)
    ...
    def _send_telemetry(self):
        ...
        telemetry = {
            ...
            "arm_state": self.arm.current_state.name if hasattr(self.arm, 'current_state') else "UNKNOWN"
        }
...
# --- Main Entry Point ---
def main():
    parser = argparse.ArgumentParser(description='ROV Control System with PID Stabilization and Arm Control')
    ...
    # Example usage: python chatPIDWeighted.py --disable-heading-pid --pid-weight 0.8
