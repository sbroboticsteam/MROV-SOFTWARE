...
class ROV:
    ...
    def __init__(self, pid_enabled=True):
        ...
        # Load PID calibration file
        self.load_pid_calibration()
    ...
    def load_pid_calibration(self):
        """Load PID gains from auto-calibration file."""
        try:
            calibration_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pid_auto_calibration.json")
            logger.info(f"Loading PID calibration from: {calibration_path}")
            
            with open(calibration_path, 'r') as f:
                calibration = json.load(f)
            
            # Set PID gains from calibration file
            self.pid_system.pid_roll = PID_Controller(
                calibration["roll_pid"]["Kp"],
                calibration["roll_pid"]["Kd"],
                calibration["roll_pid"]["Ki"],
                0.1  # filter value
            )
            
            self.pid_system.pid_pitch = PID_Controller(
                calibration["pitch_pid"]["Kp"],
                calibration["pitch_pid"]["Kd"],
                calibration["pitch_pid"]["Ki"],
                0.1
            )
            
            self.pid_system.pid_yaw = PID_Controller(
                calibration["heading_pid"]["Kp"],
                calibration["heading_pid"]["Kd"],
                calibration["heading_pid"]["Ki"],
                0.1
            )
            
            logger.info(f"Loaded PID calibration values: "
                        f"Roll(P={calibration['roll_pid']['Kp']}, I={calibration['roll_pid']['Ki']}, D={calibration['roll_pid']['Kd']}), "
                        f"Pitch(P={calibration['pitch_pid']['Kp']}, I={calibration['pitch_pid']['Ki']}, D={calibration['pitch_pid']['Kd']}), "
                        f"Heading(P={calibration['heading_pid']['Kp']}, I={calibration['heading_pid']['Ki']}, D={calibration['heading_pid']['Kd']})")
            
        except Exception as e:
            logger.error(f"Failed to load PID calibration file: {e}")
            logger.warning("Using default PID values instead")
            
            # Set default PID gains if calibration fails
